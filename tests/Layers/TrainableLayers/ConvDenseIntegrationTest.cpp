...
protected:
        ...
        size_t B)
    {
        ...
        std::vector<float> layer1Out = cpuForward(inputBuf, layer1->getWeightsCPU(p_ocl.getForwardBackpropQueue()), layer1->getBiasesCPU(p_ocl.getForwardBackpropQueue()), dims1);
        results["layer1ForwardOutput"] = layer1Out;

        std::vector<float> layer2Out = cpuForward(layer1Out, layer2->getWeightsCPU(p_ocl.getForwardBackpropQueue()), layer2->getBiasesCPU(p_ocl.getForwardBackpropQueue()), dims2);
        ...
        std::vector<float> inputDeltas = cpuBackpropDeltas(layer1Deltas, layer1->getWeightsCPU(p_ocl.getForwardBackpropQueue()), dims1);
        ...
        OpenCLResources &p_ocl)
    {
        ...
        q.enqueueReadBuffer(layer1Out, BLOCKING, NO_OFFSET,
        ...
        q.enqueueReadBuffer(layer2Out, BLOCKING, NO_OFFSET,
        ...
        q.enqueueReadBuffer(layer2->getDeltas(), BLOCKING, NO_OFFSET,
        ...
        q.enqueueReadBuffer(layer1->getDeltas(), BLOCKING, NO_OFFSET,
        ...
        cl::Buffer outputDeltas = cl::Buffer(p_ocl.getSharedResources()->getContext(), CL_MEM_READ_WRITE, B * layer1->getTotalInputElements() * sizeof(float));
        ...
        q.enqueueReadBuffer(outputDeltas, BLOCKING, NO_OFFSET, elems * sizeof(float), outputDeltasVec.data());
        ...
        p_ocl.getDeltaToGradientQueue().enqueueReadBuffer(
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ConvStressConfig{
            {3, 5, 7, 11}, {1, 1}, Utils::PaddingType::Same, {7, 13, 17}, 3},
        ConvStressConfig{
            {1, 7, 3, 5}, {2, 1}, Utils::PaddingType::Valid, {3, 19, 11}, 4},
        ConvStressConfig{
            {3, 3, 64, 128}, {1, 1}, Utils::PaddingType::Same, {64, 9, 9}, 2},
        ConvStressConfig{
            {5, 5, 3, 7}, {3, 3}, Utils::PaddingType::Valid, {3, 31, 29}, 5}));
