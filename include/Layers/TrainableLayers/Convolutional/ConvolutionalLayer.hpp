...
namespace Layers::Trainable
{
    ...
    public:
        ...
        Utils::Dimensions getWeightsDimensions() const final override { return Utils::Dimensions({getOutputChannels(), getInputChannels(), m_filterDimensions.getHeight(), m_filterDimensions.getWidth()}); }
        Utils::Dimensions getBiasesDimensions() const final override { return Utils::Dimensions({getOutputChannels()}); }
        ...
        void setBatchSize(const size_t p_batchSize) final override
        {
            ...
            Utils::setKernelArgs(1, m_backpropDeltasKernel, getDeltas());
            Utils::setKernelArgs(m_computeWeightsGradientsKernel, getDeltas());
            Utils::setKernelArgs(m_computeBiasesGradientsKernel, getDeltas());
        }
        ...
        Visualization::LayerVisualization buildVisualization(const cl::CommandQueue &p_queue, const Visualization::VisualizationOptions &p_options, size_t p_batchSize) const final override { return buildConvolutionalLayerVisualization(p_queue, p_options, p_batchSize); }
    ...
    private:
        ...
        {
            if (!trainableLayerEquals(p_queue, p_other))
                return false;
        ...
        Visualization::LayerVisualization buildConvolutionalLayerVisualization(const cl::CommandQueue &p_queue, const Visualization::VisualizationOptions &p_options, size_t p_batchSize) const
        {
            Visualization::LayerVisualization visualization = buildTrainableLayerVisualization(p_queue, p_options, p_batchSize);
            visualization.m_metadata.push_back("Filter Dimensions: " + m_filterDimensions.toString());
            visualization.m_metadata.push_back("Stride Dimensions: " + m_strideDimensions.toString());
            visualization.m_metadata.push_back("Padding Values (Top, Bottom, Left, Right): (" + std::to_string(m_paddingValues.getTop()) + ", " + std::to_string(m_paddingValues.getBottom()) + ", " + std::to_string(m_paddingValues.getLeft()) + ", " + std::to_string(m_paddingValues.getRight()) + ")");
