...
public:
    ...
    std::vector<Visualization::LayerVisualization> visualize(
        const cl::CommandQueue &queue,
        const Visualization::VisualizationOptions &options,
        size_t batchSize) const
    {
        std::vector<Visualization::LayerVisualization> result;
        result.reserve(m_layers.size());

        for (const auto &layer : m_layers)
        {
            result.push_back(layer->buildVisualization(queue, options, batchSize));
        }
