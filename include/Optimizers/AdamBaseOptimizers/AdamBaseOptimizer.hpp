...
namespace Optimizers
{
    ...
    protected:
        ...
        {
            saveOptimizer(p_optimizerGroup);
        ...
        bool momentBuffersEqual(const cl::CommandQueue &p_queue, const AdamBaseOptimizer &p_otherAdamBase, const std::map<size_t, std::pair<size_t, size_t>> &p_momentSizes) const
        {
            for (const auto &[layerId, sizes] : p_momentSizes)
            {
                ...
                if (!compareMomentBuffers(p_queue, layerIdStr + "Weights", sizes.first, m_momentBuffers, p_otherAdamBase.m_momentBuffers) ||
                    !compareMomentBuffers(p_queue, layerIdStr + "Biases", sizes.second, m_momentBuffers, p_otherAdamBase.m_momentBuffers))
                    return false;
            }
            return true;
        }
                                  ...
                                  const std::map<std::string, std::pair<cl::Buffer, cl::Buffer>> &p_buffers2) const
        {
            ...
            std::vector<float> data1 = Utils::readCLBuffer(p_queue, it1->second.first, NO_OFFSET, p_size);
            std::vector<float> data2 = Utils::readCLBuffer(p_queue, it2->second.first, NO_OFFSET, p_size);
            ...
            std::vector<float> data1_v = Utils::readCLBuffer(p_queue, it1->second.second, NO_OFFSET, p_size);
            std::vector<float> data2_v = Utils::readCLBuffer(p_queue, it2->second.second, NO_OFFSET, p_size);
            for (size_t i = 0; i < p_size; ++i)
            {
