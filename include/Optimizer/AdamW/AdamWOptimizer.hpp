 ...
 * @brief Implements the AdamW optimization algorithm.
 *
 * AdamW is a variant of the Adam optimizer that decouples weight decay
 * from the adaptive learning rate mechanism, often leading to better
 * generalization performance compared to standard Adam with L2 regularization.
 */
...
public:
     ...
     * This parameter controls the exponential moving average of the gradients.
     */
    float beta1;

    /**
     * @brief Exponential decay rate for the second moment estimates (uncentered variance of gradients).
     * This parameter controls the exponential moving average of the squared gradients.
     */
    float beta2;
     ...
     * Incremented with each parameter update step. Used in the bias correction terms.
     */
    unsigned int t;
    ...
    /**
     * @brief Constructs a new AdamWOptimizer object.
     *
     * Initializes the base Optimizer's properties (learning rate, weight decay rate)
     * and sets AdamW-specific hyperparameters. The `weight_decay_rate` here is
     * applied as a decoupled weight decay.
     *
    ...
    AdamWOptimizer(const OpenCLSetup& ocl_setup,
        ...
        : Optimizer(ocl_setup, learning_rate, weight_decay_rate), // Initialize base class
          ...
          beta1(beta1), beta2(beta2), epsilon(epsilon), t(1) {} // Initialize AdamW-specific params and time step

    // --- Destructor ---

    /**
     * @brief Destroys the AdamWOptimizer object.
    ...
    /**
     * @brief Updates the parameters (weights or biases) using the AdamW optimization algorithm.
     *
     ...
     * It specifically applies the decoupled weight decay.
     *
    ...
    /**
     * @brief Prints the configuration and current state of the AdamW optimizer to the console.
     *
