...
AdamWOptimizer::AdamWOptimizer(std::shared_ptr<Utils::SharedResources> p_sharedResources,
...
bool AdamWOptimizer::equals(const cl::CommandQueue& p_queue, const Optimizer& p_other, std::map<size_t, std::pair<size_t, size_t>>& p_momentsSizes) const {
    if (p_other.getType() != Utils::OptimizerType::AdamW) {
        return false;
    }
    const AdamWOptimizer& otherAdamW = static_cast<const AdamWOptimizer&>(p_other);

    if (m_learningRate != otherAdamW.m_learningRate ||
        m_weightDecayRate != otherAdamW.m_weightDecayRate ||
        m_beta1 != otherAdamW.m_beta1 ||
        m_beta2 != otherAdamW.m_beta2 ||
        m_epsilon != otherAdamW.m_epsilon) {
        return false;
    }

    if (m_momentBuffers.size() != otherAdamW.m_momentBuffers.size()) {
        return false;
    }
    ...
    for (const auto& [layerId, sizes] : p_momentsSizes) {
        ...
        if (!compareBuffers(p_queue, layerIdStr + "Weights", sizes.first, m_momentBuffers, otherAdamW.m_momentBuffers)) {
            return false;
        }
        
        if (!compareBuffers(p_queue, layerIdStr + "Biases", sizes.second, m_momentBuffers, otherAdamW.m_momentBuffers)) {
            return false;
        }
    }
...
cl::Event AdamWOptimizer::updateParameters(const cl::CommandQueue& p_concurrentQueue, 
...
void AdamWOptimizer::saveOptimizer(const cl::CommandQueue& p_queue, H5::Group& p_optimizerGroup,
