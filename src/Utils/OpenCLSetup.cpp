...
std::vector<std::string> OpenCLSetup::getAllKernelFiles(const std::string& folderPath) {
    ...
    // Attempt to iterate through the directory and find kernel files.
    try {
        // Use recursive_directory_iterator to find files in subdirectories as well.
        for (const auto& entry : std::filesystem::recursive_directory_iterator(folderPath)) {
            // Check if the current entry is a regular file and has a ".cl" extension.
            if (entry.is_regular_file() && entry.path().extension() == ".cl") {
                filePaths.push_back(entry.path().string()); // Add the file's full path to the list.
            }
        }
...
OpenCLSetup OpenCLSetup::createOpenCLSetup(const std::string& kernelsPath){
    ...
    cl::Platform::get(&platforms); // Get all available OpenCL platforms.
    if (platforms.empty()) {
        throw std::runtime_error("No OpenCL platforms found. Please ensure OpenCL drivers are installed.");
    }
    cl::Platform platform = platforms[0]; // Use the first platform found.
    ...
    for (int i = 0; i < devices.size(); ++i) {
        std::cout << "Device " << i << ": ";
        std::cout << "Type: " << (devices[i].getInfo<CL_DEVICE_TYPE>() == CL_DEVICE_TYPE_GPU ? "GPU" : "CPU") << ", "; // Corrected to check for CPU as well
    ...
    cl::Context context(device); // Create an OpenCL context for the selected device.
    cl::CommandQueue queue(context, device); // Create a command queue for the context and device.
